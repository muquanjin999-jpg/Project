...
public class GameManager {
    ...
    public GameManager() {
        this(GameRules.defaultRules(), System.nanoTime());
    }
    ...
    private Player<Card> buildPlayer(String id, String name, TilePos avatarPos) {
        ...
        Deck<Card> deck = buildOpeningSafeDeck(id);
    ...
    private Deck<Card> buildOpeningSafeDeck(String playerId) {
        List<Card> cards = FixedStoryDeckFactory.buildDeckFor(playerId);
        java.util.Collections.shuffle(cards, random);

        int openingHandSize = game.model.GameState.P1.equals(playerId)
                ? rules.getInitialHandP1()
                : rules.getInitialHandP2();

        ensureAffordableOpeningCard(cards, openingHandSize, 2);
        return new Deck<>(cards);
    }

    private void ensureAffordableOpeningCard(List<Card> cards, int openingHandSize, int maxCost) {
        int front = Math.min(openingHandSize, cards.size());

        for (int i = 0; i < front; i++) {
            if (cards.get(i).getCost() <= maxCost) return;
        }

        for (int i = front; i < cards.size(); i++) {
            if (cards.get(i).getCost() <= maxCost) {
                Card affordable = cards.remove(i);
                cards.add(0, affordable);
                return;
            }
        }
    }
